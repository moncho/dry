...
var callback = func(ctx context.Context, event events.Message) error { return nil }
	...
	}{
		{
			...
			fields{
				map[SourceType][]EventCallback{
					ContainerSource: []EventCallback{callback},
				},
			},
		},
...
func Test_notifyCallbacks(t *testing.T) {
	...
	var callback = func(invocations map[SourceType]int) EventCallback {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, s := range tt.args.sources {
				tt.args.r.Register(s, callback(invocations))
			}
